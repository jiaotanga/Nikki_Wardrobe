...
from agent.query_expander import QueryExpansion
...
async def recommend(message: cl.Message) -> None:
    ...
    if reply.display_mode == "item_list":
        ...
        reply_message.content = (
            f"{header}\n\n{_format_query(reply.query)}"
            f"{_format_query_expansions(reply.query_expansions)}"
        )
        await reply_message.update()
...
def _format_query_expansions(
    expansions: tuple[QueryExpansion, ...],
) -> str:
    if not expansions:
        return ""
    lines = ["query_expansions:"]
    for expansion in expansions:
        lines.extend(
            (
                f"detail_query: {expansion.detail_query}",
                "detail_keywords: "
                f"{json.dumps(expansion.detail_keywords, ensure_ascii=False)}",
            )
        )
    return "\n\n" + "\n".join(lines)
